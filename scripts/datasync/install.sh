...
if [ -f "$SPATH/datasync.sh" ] && [ -f "$SPATH/datasync.service" ] && [ -f "$SPATH/datasync.timer" ]; then
	...
	chmod +x /usr/local/sbin/datasync
	cp -v $SPATH/datasync.service /etc/systemd/system
	cp -v $SPATH/datasync.timer /etc/systemd/system
elif [ "$REMOTE_URL" != "" ]; then
	wget -nv -O /usr/local/sbin/datasync "$REMOTE_URL/datasync.sh" && \
	chmod +x /usr/local/sbin/datasync
	wget -nv -O /etc/systemd/system/datasync.service "$REMOTE_URL/datasync.service"
	wget -nv -O /etc/systemd/system/datasync.timer "$REMOTE_URL/datasync.timer"
else
	exit 1
fi
