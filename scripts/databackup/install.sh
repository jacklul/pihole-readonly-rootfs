...
SPATH=$(dirname $0)
REMOTE_URL=https://raw.githubusercontent.com/jacklul/pihole-read-only-rootfs/master/scripts/databackup

if [ -f "$SPATH/databackup.sh" ] && [ -f "$SPATH/databackup.service" ] && [ -f "$SPATH/databackup.timer" ]; then
	cp -v $SPATH/databackup.sh /usr/local/sbin/databackup && \
	chmod +x /usr/local/sbin/databackup
	
	if [ ! -f "/etc/databackup.conf" ]; then
		cp -v $SPATH/databackup.conf /etc/databackup.conf
	fi
	
	cp -v $SPATH/databackup.service /etc/systemd/system
	cp -v $SPATH/databackup.timer /etc/systemd/system
elif [ "$REMOTE_URL" != "" ]; then
	wget -nv -O /usr/local/sbin/databackup "$REMOTE_URL/databackup.sh" && \
	chmod +x /usr/local/sbin/databackup
	...
	wget -nv -O /etc/systemd/system/databackup.service "$REMOTE_URL/databackup.service"
	wget -nv -O /etc/systemd/system/databackup.timer "$REMOTE_URL/databackup.timer"
else
	exit 1
fi
